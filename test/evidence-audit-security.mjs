...
function buildRound(root) {
  ...
  writeJson(path.join(rd, "meta.json"), {
    ...
    summary: "fixture",
    versions: { skill: "1.0.0", recorder: "1", agentBrowser: "0.32.1", node: process.versions.node },
    completeness: { video: "complete", dom: "complete", network: "complete", gaps: [] },
  });
...
function withProject(fn) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "ar-evidence-symlink-"));
  try { fn(root); }
