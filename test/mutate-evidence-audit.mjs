...
const MUTATIONS = [
  ...
  },
  {
    ...
    breaks: ["missing required JSON fails"],
  },
  {
    label: "evidence-malformed-required-json-accepted",
    file: "audit",
    from: "  catch { issue(checks, `json-${name}`, \"fail\", `${name} is not valid JSON`); return { file, value: null }; }",
    to: "  catch { return { file, value: null }; }",
    breaks: ["malformed required JSON fails"],
  },
  {
  ...
  },
  {
    ...
    breaks: ["declared missing video permits absence"],
  },
  {
    ...
    breaks: ["video contradicting declared missing state warns"],
  },
  {
    label: "evidence-undeclared-missing-video-accepted",
    file: "audit",
    from: "    if (!declaredMissing) issue(checks, \"video-required\", \"fail\", \"video.webm is missing while completeness.video is not missing\");",
    to: "    if (false) issue(checks, \"video-required\", \"fail\", \"video.webm is missing while completeness.video is not missing\");",
    breaks: ["undeclared missing video fails"],
  },
  {
    label: "evidence-meta-time-order-ignored",
    file: "audit",
    from: "  if (started !== null && ended !== null && ended < started) issue(checks, \"meta-time-order\", \"fail\", \"meta.json endedAt is before startedAt\");",
    to: "  if (false) issue(checks, \"meta-time-order\", \"fail\", \"meta.json endedAt is before startedAt\");",
    breaks: ["meta timestamps must be readable and ordered"],
  },
  {
    label: "evidence-meta-summary-bounds-ignored",
    file: "audit",
    from: "  if (typeof meta.summary !== \"string\" || meta.summary !== meta.summary.trim() || /[\\r\\n]/.test(meta.summary) || meta.summary.length > 160) {",
    to: "  if (false) {",
    breaks: ["meta summary must remain a bounded single line"],
  },
  {
    label: "evidence-completeness-vocabulary-ignored",
    file: "audit",
    from: "    if (!COMPLETENESS.has(meta.completeness[channel])) {",
    to: "    if (false) {",
    breaks: ["completeness channel values must be recognized"],
  },
  {
    label: "evidence-completeness-gaps-shape-ignored",
    file: "audit",
    from: "  if (!Array.isArray(meta.completeness.gaps)) issue(checks, \"completeness-gaps\", \"fail\", \"completeness.gaps must be an array\");",
    to: "  if (false) issue(checks, \"completeness-gaps\", \"fail\", \"completeness.gaps must be an array\");",
    breaks: ["completeness gaps must be an array"],
  },
  {
    label: "evidence-dom-segments-shape-ignored",
    file: "audit",
    from: "  if (!Array.isArray(dom.segments)) issue(checks, \"dom-segments\", \"fail\", \"dom.json segments must be an array\");",
    to: "  if (false) issue(checks, \"dom-segments\", \"fail\", \"dom.json segments must be an array\");",
    breaks: ["dom segments must be an array"],
  },
  {
    ...
    breaks: ["v1 network completeness requires HAR presence"],
  },
  {
    label: "evidence-malformed-har-accepted",
    file: "audit",
    from: "  } catch { issue(checks, \"network-har-json\", \"fail\", \"network.har is not valid JSON\"); }",
    to: "  } catch { }",
    breaks: ["present v1 HAR must be valid JSON"],
  },
  {
    ...
    breaks: ["v2 must not contain network HAR"],
  },
  {
    ...
    breaks: ["v2 network completeness must be missing"],
  },
  {
    label: "evidence-v2-backend-identity-ignored",
    file: "audit",
    from: "    if (!isObject(meta.versions) || meta.versions.backend !== \"browser-control\") {",
    to: "    if (false) {",
    breaks: ["v2 requires browser-control backend identity"],
  },
  {
    ...
    breaks: ["missing comment JPEG sidecar fails"],
  },
  {
    ...
    breaks: ["invalid comment JPEG sidecar fails"],
  },
  {
    label: "evidence-invalid-comment-id-silenced",
    file: "audit",
    from: "      issue(checks, \"comment-id\", \"fail\", \"comment id is not canonical\");",
    to: "      // mutation: invalid comment id silently skipped",
    breaks: ["noncanonical comment id fails"],
  },
  {
    ...
    breaks: ["resolution key must belong to a comment"],
  },
  {
    label: "evidence-resolution-target-id-ignored",
    file: "audit",
    from: "    if (typeof item.resolvedInRoundId !== \"string\" || !ROUND_ID_RE.test(item.resolvedInRoundId)) issue(checks, \"resolution-round\", \"fail\", `resolution ${commentId} has invalid resolvedInRoundId`);",
    to: "    if (false) issue(checks, \"resolution-round\", \"fail\", `resolution ${commentId} has invalid resolvedInRoundId`);",
    breaks: ["resolution target round id must be canonical"],
  },
  {
    label: "evidence-resolution-time-ignored",
    file: "audit",
    from: "    if (!validTime(item.resolvedAt)) issue(checks, \"resolution-time\", \"fail\", `resolution ${commentId} has invalid resolvedAt`);",
    to: "    if (false) issue(checks, \"resolution-time\", \"fail\", `resolution ${commentId} has invalid resolvedAt`);",
    breaks: ["resolution timestamp must be readable"],
  },
  {
  ...
  },
  {
    ...
    breaks: ["unknown top-level files warn"],
  },
  {
    label: "evidence-orphan-image-warning-silenced",
    file: "audit",
    from: "    if (!commentIds.has(id)) issue(checks, \"orphan-comment-image\", \"warn\", `orphan comment image ${entry.name}`);",
    to: "    if (false) issue(checks, \"orphan-comment-image\", \"warn\", `orphan comment image ${entry.name}`);",
    breaks: ["orphan JPEGs warn"],
  },
  {
  ...
  {
    label: "evidence-missing-round-filter-accepted",
    file: "audit",
    from: "  const selected = roundId ? [roundId] : available;\n  if (roundId && !available.includes(roundId)) issue(checks, \"round-filter\", \"fail\", `requested round ${roundId} does not exist`);",
    to: "  const selected = roundId && available.includes(roundId) ? [roundId] : available;\n  if (false) issue(checks, \"round-filter\", \"fail\", `requested round ${roundId} does not exist`);",
    breaks: ["missing round filter fails"],
  },
  {
  ...
  },
  {
    ...
    breaks: ["evidence CLI rejects missing project value"],
  },
  {
    ...
    breaks: ["evidence CLI rejects unexpected arguments"],
  },
];
