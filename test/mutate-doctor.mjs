...
const MUTATIONS = [
  ...
  {
    label: "doctor-browser-control-status-not-json",
    file: "diagnostics",
    from: "  const statusProbe = exec({ bin: \"browser-control\", args: [\"status\", \"--json\"], timeoutMs: 10000 });",
    to: "  const statusProbe = exec({ bin: \"browser-control\", args: [\"status\"], timeoutMs: 10000 });",
    breaks: ["browser-control diagnostics require its CLI and ffmpeg"],
  },
  {
  ...
  {
    label: "doctor-browser-control-stale-relay-accepted",
    file: "diagnostics",
    from: "  if (status.relay.stale === true) {",
    to: "  if (false) {",
    breaks: ["browser-control mirrors relay and extension fail-closed gates"],
  },
  {
    label: "doctor-browser-control-disconnected-extension-accepted",
    file: "diagnostics",
    from: "  if (status.extension.connected !== true) {",
    to: "  if (false) {",
    breaks: ["browser-control mirrors relay and extension fail-closed gates"],
  },
  {
    label: "doctor-browser-control-doctor-not-json",
    file: "diagnostics",
  ...
  },
  {
    ...
    file: "diagnostics",
    from: "  if (doctor.extension.versionMatches === false) {",
    to: "  if (false) {",
    breaks: ["browser-control refuses a confirmed extension version mismatch"],
  },
  {
    label: "doctor-browser-control-unknown-version-upgraded-to-pass",
    file: "diagnostics",
    from: "  if (doctor.extension.versionMatches !== true) {",
    to: "  if (false) {",
    breaks: ["browser-control undetermined extension version warns without leaking raw output"],
  },
  {
    label: "doctor-browser-control-raw-output-leaked",
    file: "diagnostics",
    from: "    checks.push({ id: \"browser-control-doctor\", status: \"warn\", message: \"extension version could not be confirmed\" });",
    to: "    checks.push({ id: \"browser-control-doctor\", status: \"warn\", message: String(doctorProbe.stdout || \"\") });",
    breaks: ["browser-control undetermined extension version warns without leaking raw output"],
  },
  {
