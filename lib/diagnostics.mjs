...
function commandMissing(result) {
  return !result || result.status === null || result.errorCode === "ENOENT";
}
...
function agentBrowserChecks(exec) {
  ...
  if (!commandSucceeded(version)) {
    checks.push({ id: "agent-browser", status: "fail", message: "version probe failed" });
    return checks;
  }
  ...
  checks.push(commandSucceeded(record)
    ...
    : { id: "recording", status: "fail", message: "record command unavailable" });
  ...
  checks.push(commandSucceeded(network)
    ...
    : { id: "network", status: "fail", message: "network har command unavailable" });
  return checks;
}
...
function browserControlChecks(exec) {
  ...
  if (!commandSucceeded(version)) {
    checks.push({ id: "browser-control", status: "fail", message: "version probe failed" });
    return checks;
  }
  ...
  if (!commandSucceeded(doctor)) {
    checks.push({ id: "browser-control-doctor", status: "fail", message: "browser-control doctor failed" });
    return checks;
  }
